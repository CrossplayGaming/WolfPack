...
def gen_for_set(setname):
    ...
    for lname, rgb, size, up in lights:
        if lname in seen:
            continue
        seen.add(lname)
        gl.append(f"pointlight {lname}\n{{\n"
                  f"    color {rgb[0]:.2f} {rgb[1]:.2f} {rgb[2]:.2f}\n"
                  f"    size {size}\n    offset 0 {up} 0\n"
                  f"    attenuate 1\n}}")
    for cname, sprname, lname in objects:
        gl.append(f"object {cname}\n{{\n    frame {sprname}\n    {{\n"
                  f"        light {lname}\n    }}\n}}")
    for cname, sprname, lname, rgb, size in PROJECTILES:
        if rgb and lname not in seen:
            seen.add(lname)
            gl.append(f"pointlight {lname}\n{{\n"
                      f"    color {rgb[0]:.2f} {rgb[1]:.2f} "
                      f"{rgb[2]:.2f}\n    size {size}\n"
                      f"    attenuate 1\n}}")
        gl.append(f"object {cname}\n{{\n    frame {sprname}\n    {{\n"
                  f"        light {lname}\n    }}\n}}")
    # pool-less sprites must glow too: after the swap the actor renders
    # XPnn, and a frame-S### attachment no longer matches it
    for line in swap:
        cname, xp = line.split()
